...
function sec_session_start() {
	...
	$httponly = true;
	// Forces sessions to only use coolies.
	...
	if($stmt = $mysqli->prepare("SELECT id, username, password
		...
		if($stmt->num_rows == 1) {
			...
			if(checkbrute($user_id, $mysqli) == TRUE) {
				...
				// Check if the password in database matches.
				...
				if(password_verify($password, $db_password)) {
					...
					// record this attempt to database.
					$now = time();
...
function checkbrute($user_id, $mysqli) {
	...
	$now = time();
	// All login attempts are counted from the past 5 hours.
...
function login_check($mysqli) {
	...
	if(isset($_SESSION['user_id'], $_SESSION['username'], $_SESSION['login_string'])) {
		...
		if($stmt->num_rows == 1) {
			// If the user exists get variables from the result.
